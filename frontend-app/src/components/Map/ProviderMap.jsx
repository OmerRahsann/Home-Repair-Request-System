      ...
      {requests.length && requests.map((request, i) => (
          ...
          >
                <Paper elevation={3} className={classes.paper}>
                  <h1 className='text-[1.75vh] font-bold text-center pb-1'> {request.title}</h1>
                  <img
                 ...
                 {console.log(request.pictures)}
                  <h1 className='pt-1 text-center text-green-600 font-bold'>${createRoundedRange(request.dollars)}</h1>
                </Paper>
          </div>
        ))}
      </GoogleMap>) : (<div className='flex items-center justify-center h-full'><CircularProgress/></div>)}
    </div>
  )
}
