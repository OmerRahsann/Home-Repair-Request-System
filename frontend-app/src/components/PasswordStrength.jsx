const passwordRules = []
const addPasswordRule = (label, predicate) =>
  passwordRules.push({ label, predicate })
addPasswordRule(
...
addPasswordRule(
  ...
  (password) => password.search(/[0-9]/) != -1,
)
...
function checkPasswordRequirements(password) {
  ...
  for (let i = 0; i < passwordRules.length; i++) {
    ...
    rows.push(
      <p
        className={satisfied ? 'text-green-500' : 'text-red-500'}
        key={'req' + i}
      >
...
export default function PasswordStrength({
  ...
  return (
    <div>
      {confirmPassword != '' && (
        <p className={passwordsMatch ? 'text-green-500' : 'text-red-500'}>
